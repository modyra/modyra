 ...
 */
/** Source with its comments removed: a name written in prose answers a presence check for code that does nothing. */
...
const catalog = ["packages/widgets/src/catalog.ts", "packages/widgets/src/catalog/contracts.ts", "packages/widgets/src/catalog/define.ts"]
  .map((path) => { try { return strip(readFileSync(resolve(root, path), "utf8")); } catch { return ""; } })
  .join("\n");
const controller = strip(readFileSync(resolve(root, "packages/widgets/src/catalog-controller.ts"), "utf8"));
const failures = [];
for (const [kind, evidence] of Object.entries(matrix.widgets)) {
  ...
  const angular = strip(readFileSync(resolve(root, evidence.angularRenderer), "utf8"));
...
console.log("Read from source with its comments removed: which names each file contains, never what it renders.");
