...
const DELIBERATELY_OUTSIDE = new Map([
  ...
  ["test:part-addressing", "names the popup doors and panels two kinds spell the same way, which a page-wide probe cannot tell apart - ADR 0131 makes those queries page-wide, so there is no root to scope to. The four it names today are repairable only in the contract, by a session that owns the widgets package: wiring it in now would refuse every commit for a defect its author cannot fix from where they stand, and the way around a refusal is --no-verify, which costs the check its credibility for everything else. Its condition is its own output: when the list is empty, delete this line and put it in the gate chain"],
  ...
  ["lint", "run by no workflow and currently failing, and not because of any recent change - it has simply never gated anything. Wiring it in before it is green would put main red on the first push, so it stays outside until the files are clean. How far off it is belongs to `npm run lint`, not to this line: a count written here goes on being read as a measurement long after the batches have moved it, which is how it came to overstate the debt fivefold. This exemption is temporary and its condition needs no number: when `npm run lint` exits zero, delete this line and give it a workflow"],
