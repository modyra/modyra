...
function publicNames() {
  refuseStaleBuilds(PACKAGES, { root: ROOT, reads: "the declarations a package emits" });
...
function membersOf(node) {
  ...
  for (const member of node.members ?? []) {
    const name = member.name && ts.isIdentifier(member.name)
      ...
      : member.name?.getText?.();
    if (!name) continue;
    const type = member.type === undefined ? "(inferred)" : normaliseSpacing(member.type.getText?.() ?? "(inferred)");
...
function parseMembers(entries) {
  ...
  for (const entry of entries) {
    ...
    // Canonicalised on both sides, so a baseline recorded before this tool sorted unions compares
