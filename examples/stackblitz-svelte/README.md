# modyra × Svelte (StackBlitz)

Signup form built with the **published** `@modyra/svelte` package on Vite —
no monorepo, no workspace, `npm install` and go.

[![Open in StackBlitz](https://developer.stackblitz.com/img/open_in_stackblitz.svg)](https://stackblitz.com/github/modyra/modyra/tree/main/examples/stackblitz-svelte)
...
- `createSvelteForm` + `toStore()` — typed fields bridged to real Svelte
  `Readable` stores, `$store` auto-subscription in the template
