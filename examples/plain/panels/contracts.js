...
export const contractsPanel = {
  ...
  mount(work, readout) {
    ...
    for (const door of MDY_CLASS_DOORS) {
      ...
      // "Asked with nothing" and "answered with nothing" are different rows, and the difference is
      // the contract's to state rather than this page's to remember: `SAMPLE_CALL` is written by
      // hand, so the door added tomorrow is exactly the one that arrives without an entry, and a
      // second page would have to rediscover the distinction. `answerDoor` says which it was, so the
      // row below reads the reason it is given instead of inventing one.
