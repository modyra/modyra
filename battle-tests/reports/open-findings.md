...
S2     2
      --
       5      open reds, 2026-08-20 13:25:43 UTC
```
...
Driving the parser with one document per ordinary mistake, plus every published fixture, **seven**
codes come back that the list does not have:
...
| `MDY_DYNAMIC_INVALID_NODE` | a node whose `node` is a word the contract does not have |
| `MDY_DYNAMIC_INVALID_RECORD` | a `record` with no `item`, or an `item` that is not a node |
| `MDY_DYNAMIC_INVALID_ARRAY` | an `array` whose `initialValue` is not a list |

The published list has seven entries. **It describes half of what a parse can say**, and the half it
leaves out is the half about the shapes a document is built from — nodes, records, arrays, layouts,
rules. None of the seven is exotic: each is one ordinary mistake away. A consumer handling the seven and falling through on anything else meets
...
Checked alongside and *nearly* clean: the parser enforces most of the schema's own types and ranges.
Thirteen of fourteen violations are reported — a missing `kind` or `name`, a name, kind or label that is not a
...
**Why this is worth a finding rather than a typo.** CLAUDE.md makes a decision record load-bearing:
...
to say and why that was dangerous — which is the form CLAUDE.md requires: a record is superseded or
...
`CLAUDE.md` § Comments and documentation forbids exactly one of the things these comments do:
...
directory that CLAUDE.md itself says must never be the sole durable record of a decision. None of the
...
nothing they can open. The rule CLAUDE.md gives for this case is in the same section: express a real
...
**The neighbouring clause of the same rule is in far better shape.** CLAUDE.md also forbids change
...
## 227 — What the published schema refuses, the parser does not (S1, DYN-004 DYN-003)

A document is checked in three places and they are meant to agree:

```
the TypeScript type      refuses     excess property check on a typed literal
the published JSON Schema refuses    17 × "additionalProperties": false in spec/dynamic-form-v3
the parser                SILENT     zero of five slots
```

Measured, one document per slot, each carrying a key the contract does not declare:
...
The two that refuse are the two a document can arrive without. A config stored in a CMS, generated by
a model, or assembled by a server was never a TypeScript literal and never passed an editor — the
parser is all it meets, and `ai-generated-forms.md` says so in as many words: *"the parser stays the
enforcement layer — prompts reduce waste, `parseDynamicFields()` enforces the supported contract."*
`apps/vscode/package.json` points every `*.form.json` at that same schema, so the line is drawn and
enforced while a document is *written* and not when it is *read*.

**Not an argument for refusing every unknown key.** A document may carry members a reader predating
them does not know, which is what lets a v3 document be read by a v2 parser. The assertion takes its
boundary from the repository rather than from an opinion: **what the published JSON Schema refuses,
the parser reports too.** And the sentence already exists here twice — `createForm` and, since
finding 226, `field()`.

Pinned by `adversarial/dynamic-contract/a-key-the-schema-refuses.battle.test.mjs`, with a control
