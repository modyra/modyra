...
/**
 * The operations a browser battle drives, with the one adapter-specific step left to the caller.
 *
 * Three hosts answered the same 193 specs by repeating the same three hundred lines: the guards, the
 * parser, the validators, the layout, and sixteen doors that only ever touch `MdyTypedForm` — whose
 * signature is identical across every adapter, measured on four factories. What actually differed
 * was one step: how a single field becomes an element on the page. Lit creates a registered element
 * and sets `.field`; Vue mounts a component with the handle as a prop.
 *
 * So that step is the parameter and the rest is here, once. It was **lifted, not rewritten**: a core
 * written afresh would re-implement the semantics those specs encode, and a subtle divergence would
 * surface as "this renderer behaves differently" — a red accusing the product for a defect in the
 * bench. Lit's behaviour is preserved by construction, because this is lit's code.
 *
 * @param {{
 *   createForm: (schema: unknown, options: unknown) => unknown,
 *   mountOneField: (spec: {
 *     declared: Record<string, unknown>, handle: unknown, into: Element,
 *     idPrefix?: string | null,
 *   }) => Element,
 *   errorSummaryElement?: () => Element | null,
 *   tagFor?: (kind: string) => string | null,
 * }} adapter
 */
export function createBattleHost(adapter) {
  const { createForm, mountOneField, errorSummaryElement, tagFor } = adapter;
  ...
  mountFields(id, fields, options = {}) {
    ...
    try {
      ...
      for (const declared of parsed) {
        ...
        // the handle as a prop. Everything either side of this line is the same question.
        //
