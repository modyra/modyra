...
- [ADR 0182: Where the text a person types begins is declared once](0182-where-the-text-a-person-types-begins.md) — the inset is declared on the inliner and on the control and both apply, so one renderer pays it twice; declared once, applied once, at the number two renderers already draw — **amended**: a kind whose inliner does not carry the inset is not ready for it, and two ways a correct cascade rule loses
