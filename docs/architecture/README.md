...
- [ADR 0180: A container held open under a field that can fail](0180-a-container-held-open-under-a-field-that-can-fail.md) — the message's container is present from the first paint, so the page does not move under a person and one reference stands; a named empty container is not a dangling one — **amended**: the complement, when an empty container may go, and why an empty flex child is still charged a gap
...
- [ADR 0182: Where the text a person types begins is declared once](0182-where-the-text-a-person-types-begins.md) — the inset is declared on the inliner and on the control and both apply, so one renderer pays it twice; declared once, applied once, at the number two renderers already draw — **amended**: a colour field hides a native picker ahead of its hex box, so a rule about "the control inside the wrapper" reaches an element nobody writes in; that and two ways a correct cascade rule loses
