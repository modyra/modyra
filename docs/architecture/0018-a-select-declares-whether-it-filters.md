...
**A native `<select>` satisfies the listbox model.** It is a listbox, it has typeahead, and its
typeahead is the platform's rather than a reimplementation of it — `@modyra/lit` renders one for
every non-searchable select and is right to. The model describes the *behaviour* a user gets, not the
markup: requiring a custom trigger and a portalled list would be a rule against the best available
implementation. What the model forbids is a third behaviour, which is what existed before it.
...
- **`@modyra/lit`'s typeahead starts working** on its searchable path, where the bug was: typing on
  the trigger dispatched one character at a time into a controller that replaces the query. Its
  non-searchable select is a native chooser and always had the platform's typeahead, which is why
  the defect was invisible in the model most selects use.
