...
|---|---|---|---|---|---|---|---|---|---|---|---|
| vanilla | yes | yes | yes | no | yes | yes | yes | — | no | no | reference implementation |
...
| react | yes | yes | yes | no | yes | yes | yes | — | no | no | = vanilla (no signal primitive of its own) |
| preact | yes | yes | yes | no | yes | yes | yes | — | no | no | = vanilla (no signal primitive of its own) |
| svelte | yes | yes | yes | no | yes | yes | yes | — | no | no | = vanilla (no signal primitive of its own); toStore() bridges to a Readable |
| lit | yes | yes | yes | no | yes | yes | yes | — | no | no | = vanilla (no signal primitive of its own) |
| vue | yes | yes | yes | no | yes | yes | yes | — | no | no | native @vue/reactivity; effect() scheduler + createScope() via effectScope() (the current implementation) |
| solid | yes | yes | yes | yes | yes | yes | yes | — | no | no | native createSignal/createMemo/createEffect; equals comparator honored on both signal and memo (the current implementation) |
