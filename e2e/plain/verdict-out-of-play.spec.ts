 ...
 * renderer stopped marking. This drives the laboratory's state panel and reads the page back.
 */
...
async function readout(page: import("@playwright/test").Page) {
  return JSON.parse((await page.locator("[data-readout]").textContent()) ?? "{}");
}

test("the catalogue paints its verdict while the form is asking", async ({ page }) => {
  await page.goto("/lab.html#states");
