...
const rowByKey = (key: string) => `tr:has(td:text-is("${key}"))`;
...
test("sorting re-renders every cell and moves no value", async ({ page }) => {
  ...
  const firstKey = page.locator("tbody tr").first().locator("td").first();
...
test("closing every editor keeps the values and the verdict", async ({ page }) => {
  ...
  await expect(page.locator("input")).toHaveCount(0, { timeout: 2000 });
...
test("a row nobody is rendering still makes the form invalid", async ({ page }) => {
  ...
  await expect(page.locator(verdict)).toContainText("form valid: false");
