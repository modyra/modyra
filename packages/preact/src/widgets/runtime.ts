/**
 * Preact runtime for Modyra widget commands.
 */
...
/**
 * Schedules command execution after the next microtask (mirrors the React
 * adapter's layout-effect-adjacent flush).
 *
 * Returns a queue function; call it from event handlers to push commands
 * that will be flushed shortly after.
 */
export function useMdyCommandQueue(
  ...
  const announcerRef = useRef(createMdyAnnouncer("mdy-preact-announcer"));
