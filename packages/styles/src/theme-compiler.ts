...
/**
 * What a token name and a token value may not contain, for the reason a selector may not.
 *
 * A declaration is written at the position a declaration occupies. `}` ends the block, `;` ends the
 * declaration, and `<` ends the sheet where the sheet is a `<style>` block. A colour needs none of
 * them.
 */
const DECLARATION_ESCAPES = /[{};<]|\/\*|\*\//;

/**
 * Refuses a theme that cannot be written out, at the point it is written.
 *
 * {@link compileMdyTheme} guards the selector it is handed, and that is the earlier and friendlier
 * refusal — but it guards the function that *builds* the object, and this is the function that
 * *writes the sheet*. `MdyResolvedTheme` is a plain frozen object, so a caller holding tokens of its
 * own can construct one and reach here without passing the compiler at all. Containment belongs
 * where the interpolation happens or it is provenance, which a type cannot enforce at runtime.
 *
 * Every field this function interpolates is checked, including the two that only reach a comment:
 * `*&#47;` closes a comment as surely as `}` closes a rule, and what follows is parsed as CSS.
 *
 * `seed` and `model` are checked for what they are rather than for characters they lack — a hex and
 * a name the registry knows — because that is what the compiler guarantees, and a blacklist here
 * would accept values the rest of this module cannot use.
 */
function assertWritableTheme(theme: MdyResolvedTheme): void {
  if (SELECTOR_ESCAPES.test(theme.selector)) {
    throw new Error(
      `Invalid theme selector: ${theme.selector}. A selector is written into a rule inside a ` +
        "stylesheet, so it may not contain the characters that end either one.",
    );
  }
  if (!/^#[0-9a-f]{6}$/i.test(theme.seed)) {
    throw new Error(`Invalid theme seed: ${theme.seed}. It is written into the sheet's header comment.`);
  }
  if (!Object.hasOwn(MDY_PERCEPTUAL_PALETTE_MODELS, theme.model)) {
    throw new Error(
      `Unknown theme model: ${String(theme.model)}. Available: ` +
        `${Object.keys(MDY_PERCEPTUAL_PALETTE_MODELS).join(", ")}.`,
    );
  }
  for (const [mode, tokens] of [["light", theme.light], ["dark", theme.dark]] as const) {
    for (const [name, value] of Object.entries(tokens)) {
      if (!/^[A-Za-z][A-Za-z0-9]*$/.test(name)) {
        throw new Error(`Invalid ${mode} token name: ${name}. A token name becomes a custom property.`);
      }
      if (DECLARATION_ESCAPES.test(value)) {
        throw new Error(`Invalid ${mode} token value for ${name}: ${value}.`);
      }
    }
  }
}
